...
import com.example.apachepoi.hwpf.usermodel.BorderCode;
import com.example.apachepoi.hwpf.usermodel.TableAutoformatLookSpecifier;
import com.example.apachepoi.hwpf.usermodel.TableCellDescriptor;
import com.example.apachepoi.hwpf.usermodel.TableProperties;
...
public final class TableSprmCompressor {
  ...
  public static byte[] compressTableProperty(TableProperties newTAP)
  {
    ...
    if (newTAP.getJc() != 0)
    {
      size += org.apache.poi.hwpf.sprm.SprmUtils.addSprm((short)0x5400, newTAP.getJc(), null, sprmList);
    }
    if (newTAP.getFCantSplit())
    {
      size += org.apache.poi.hwpf.sprm.SprmUtils.addSprm((short)0x3403, 1, null, sprmList);
    }
    if (newTAP.getFTableHeader())
    {
      size += org.apache.poi.hwpf.sprm.SprmUtils.addSprm((short)0x3404, 1, null, sprmList);
    }
    ...
    {
      size += org.apache.poi.hwpf.sprm.SprmUtils.addSprm((short)0xD605, 0, brcBuf, sprmList);
    }
    if (newTAP.getDyaRowHeight() != 0)
    {
      size += org.apache.poi.hwpf.sprm.SprmUtils.addSprm((short)0x9407, newTAP.getDyaRowHeight(), null, sprmList);
    }
    if (newTAP.getItcMac() > 0)
    {
      ...
      byte[] buf = IOUtils.safelyAllocate(
              ...
              org.apache.poi.hwpf.sprm.SprmUtils.MAX_RECORD_LENGTH);
      buf[0] = (byte)itcMac;
      ...
      size += org.apache.poi.hwpf.sprm.SprmUtils.addSpecialSprm((short)0xD608, buf, sprmList);
...
//      size += SprmUtils.addSpecialSprm((short)0xD609, buf, sprmList);
        ...
        if ( newTAP.getTlp() != null && !newTAP.getTlp().isEmpty() )
        {
            ...
            size += org.apache.poi.hwpf.sprm.SprmUtils.addSprm( (short) 0x740a, 0, buf, sprmList );
        }
